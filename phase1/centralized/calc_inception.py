...
def extract_features(loader, inception, device):
    ...
    for img, label, zaxis in pbar:
        img = img.to(device)
        feature = inception(img)[0].view(img.shape[0], -1)
        feature_list.append(feature.to("cpu"))
        labels_list.append(label.numpy())
        zaxes_list.append(zaxis.numpy())

    utils.save_image(
        img,
        f"inception_debug.png",
    ...
    return features, labels_list, zaxes_list
...
if __name__ == "__main__":
    ...
    features, labels, zaxes = extract_features(loader, inception, device)
    ...
    with open(f"inception_{name}.pkl", "wb") as f:
        pickle.dump({"mean": mean, "cov": cov, "size": args.size, "path": args.path, "labels": labels, "zaxes": zaxes}, f)
